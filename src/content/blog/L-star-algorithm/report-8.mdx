...
**Proposition 3.** Suppose that $(I,P,S)$ is separable, closed and consistent. Let $\mathcal{M} = (Q,\Sigma,\delta,l/r,i_0,F)$ be the hypothesis automaton. Given a counterexample $w = w_1 . . . w_n$ to $\mathcal{M}$, using membership queries, one can find $i ∈ Σ^∗ \ I$ and $s ∈ Σ^∗$ such that $(I ∪ {i}, S ∪ {s})$ is separable. 
...
**Proposition 4.** *Assume that $(I,P,S)$ is closed and separable observation table and the algorithm terminated. Then the acceptor $\mathcal{M} = (Q,\Sigma,\delta,l/r,q_0,F)$ is the syntactic monoid of the language $L$​.*
...
Then we have $\delta(\delta^*(q_0,aw),b,r) = \delta(\delta^*(q_0,aw'),b,r)$. It is same if we concat $b$ at right side first.
